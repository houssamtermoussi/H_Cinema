...
<template>
    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-950 transition-colors duration-300 p-8 md:p-16">
            <div class="max-w-7xl mx-auto">
                <header class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16">
                    <div>
                        <h1 class="text-5xl font-black text-gray-900 dark:text-white mb-4 tracking-tighter uppercase font-cinzel">MES RÉSERVATIONS</h1>
                        <p class="text-gray-500 dark:text-zinc-500 font-bold text-lg">Retrouvez vos tickets et l'historique de vos séances.</p>
                    </div>
                    <div class="flex items-center gap-4">
                        <div class="px-6 py-3 bg-white dark:bg-zinc-900 rounded-2xl border border-gray-200 dark:border-zinc-800 shadow-sm">
                            <span class="block text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-1">Total</span>
                            <span class="block text-2xl font-black text-gray-900 dark:text-white leading-none">{{ reservations.length }}</span>
                        </div>
                    </div>
                </header>
                
                <div v-if="reservations.length > 0" class="space-y-6">
                    <div v-for="res in reservations" :key="res.id" class="group relative bg-white dark:bg-zinc-900/40 backdrop-blur-xl border border-gray-200 dark:border-zinc-800/50 p-8 rounded-[2.5rem] flex flex-col md:flex-row items-center justify-between gap-10 hover:bg-gray-50 dark:hover:bg-zinc-900/60 transition-all duration-500 hover:border-[#cc0000]/30 shadow-xl overflow-hidden">
                        <!-- Decorative element -->
                        <div class="absolute -right-10 -top-10 w-40 h-40 bg-[#cc0000]/5 blur-[80px] rounded-full group-hover:bg-[#cc0000]/10 transition-all duration-700"></div>
                        ...
                        <div class="flex items-center gap-8 w-full md:w-auto relative z-10">
                            ...
                            <div>
                                <div class="flex items-center gap-2 mb-2">
                                    <span class="px-2 py-0.5 bg-[#cc0000]/10 text-[#cc0000] text-[10px] font-black uppercase rounded-full border border-[#cc0000]/20 tracking-tighter">Billet #{{ res.id }}</span>
                                </div>
                                <h3 class="text-3xl font-black text-gray-900 dark:text-white mb-2 tracking-tighter group-hover:text-[#cc0000] transition-colors duration-300">{{ res.seance.film.titre }}</h3>
                        ...
                        <div class="flex items-center gap-12 w-full md:w-auto justify-between md:justify-end relative z-10">
                            <div class="text-center md:text-right">
                                ...
                                <p class="text-gray-900 dark:text-white font-black text-3xl tracking-tighter">{{ res.nombre_places }}</p>
                            </div>
                ...
                <div v-else class="flex flex-col items-center justify-center py-32 bg-white dark:bg-zinc-950 rounded-[3rem] border border-dashed border-gray-200 dark:border-zinc-800/50 shadow-sm">
                    ...
                    <h2 class="text-2xl font-black text-gray-900 dark:text-white mb-2 tracking-tight uppercase">AUCUNE RÉSERVATION TROUVÉE</h2>
                    <p class="text-gray-500 dark:text-zinc-500 font-medium mb-10">Il est temps de réserver votre première séance !</p>
