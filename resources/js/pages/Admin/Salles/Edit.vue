...
const submit = () => {
    form.put(adminRoutes.salles.update(props.salle.id).url);
};
</script>

<template>
    <Head :title="'Modifier ' + salle.nom" />
    <div class="py-10">
        <div class="max-w-3xl mx-auto px-4 md:px-8">
            <header class="mb-10 text-center">
                <h1 class="text-3xl md:text-5xl font-bold text-gray-900 dark:text-white tracking-tight font-cinzel uppercase">Modifier la Salle</h1>
                <p class="text-gray-500 dark:text-gray-400 font-medium mt-2">Mettez à jour les informations de la salle.</p>
            </header>
            ...
            <form @submit.prevent="submit" class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 p-8 rounded-2xl shadow-md space-y-6">
                ...
                <div class="pt-6 flex gap-4">
                    ...
                    >
                        {{ form.processing ? 'Mise à jour...' : 'Enregistrer les modifications' }}
                    </button>
                </div>
            </form>
        </div>
